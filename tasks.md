...
- 影响文件：`internal/external/client_test.go`、`internal/rpc/session_test.go`、`internal/rpc/jsonrpc_test.go`
- 依赖任务：无
- 参考资料定位：`Client.Initialize`、`rpcCaller`、`Session.Call`、`NewNotification`
- 内容：先覆盖初始化必填字段、服务端协商结果、初始化完成通知顺序，以及通知无需响应的行为。
...
- 影响文件：`internal/external/client.go`、`internal/external/client_test.go`、`internal/version/version.go`
- 依赖任务：1、2
...
- 影响文件：`internal/external/config.go`、`internal/external/http_transport.go`、`internal/external/manager_test.go`、`internal/e2e/external_tools_e2e_test.go`
- 依赖任务：4、5
- 参考资料定位：HTTP 非成功状态错误、配置展开错误、`Manager.Discover`
- 内容：确保配置、HTTP 状态、JSON-RPC 和逐 server 发现错误均不包含 Header 值、环境变量值或令牌；保留状态码、server 名和缺失变量名用于诊断。
...
- 内容：记录标准握手、按需发现、Header 环境变量语法和失败隔离；Context7 只给出 HTTP 配置示例，不写入真实令牌。

## 8. 接入主流程

- 影响文件：`internal/external/manager.go`、`internal/external/register.go`、`internal/app/app.go`、`internal/agent/agent.go`、对应测试文件
- 依赖任务：2-7
- 参考资料定位：`Manager.Client`、按需发现工具注册、`App.Run`、Agent 每轮工具定义构建
- 内容：将标准握手和认证 transport 接入按需发现流程，确认启动零连接、逐 server 失败隔离、成功连接缓存和后续远端工具调用保持一致。
