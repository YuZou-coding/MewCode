...
- `/permissions`、`/perms`：显示当前权限模式和用户级、项目级、会话级规则摘要；支持 `strict|default|yolo|reset`，也兼容 `mode strict|default|yolo|reset`。
- `/permissions clear-session`：清空本会话临时权限规则，不修改权限文件。
- `/skills`：查看 Skill；支持 `show <name>`、`run <name> [args]`、`reload`。
- `/workers`：查看后台 worker 任务。
- `/workers list`：列出当前生效的全部 worker 角色，包括内置、用户级和项目级角色，以及来源、描述、工具权限和隔离模式。
...
同名 worker 优先级为项目级 > 用户级 > 内置。运行中的 Agent 会在 `run_worker.role` 的工具 schema 和上下文中看到当前生效的全部角色；角色文件使用 YAML frontmatter + Markdown 正文：
...
工具执行前会经过安全检查。危险命令和项目根目录外路径会直接拒绝；第四层权限模式由 `mewcode.yaml` 的 `permission_mode` 控制：`strict` 对每次常规调用确认，`default` 按规则并在未命中时确认，`yolo` 自动允许常规调用。三种模式都不能绕过危险命令黑名单、路径沙箱或显式 deny 规则。

默认模式下，未命中明确规则时会进入人在回路确认：
...
显式 deny 规则始终优先；其他规则优先级为会话规则 > 项目规则 > 用户规则。危险命令黑名单和路径沙箱是硬边界，不能被 allow 规则绕过。

使用 `/permissions` 查看当前模式；用 `/permissions strict|default|yolo` 临时切换，用 `/permissions reset` 恢复启动配置；也兼容 `/permissions mode ...`。切换仅对当前会话有效。
...
- name: local_tools
  transport: stdio
  description: Project-local knowledge and automation tools
  capabilities: ["knowledge", "automation"]
  keywords: ["project docs", "local workflow"]
  examples: ["Search project knowledge", "Run local automation"]
...
- name: context7
  transport: http
  description: Current official library and framework documentation
  capabilities: ["docs", "library-docs", "api-reference"]
  keywords: ["react", "next.js", "typescript", "official documentation"]
  examples: ["Look up React useEffect latest docs", "Check Next.js routing API"]
...
`description`、`capabilities`、`keywords` 和 `examples` 都是可选路由元数据，只帮助 `tool_search` 判断哪个 server 更相关，不会触发启动时连接，也不会改变权限或认证行为。推荐用稳定能力词描述 server，例如 `docs`、`browser`、`database`、`github`、`issues`、`cloud`、`knowledge`、`design`、`api-reference`。

初始工具列表只包含内置 `tool_search`，不包含远端工具。模型可用自然语言任务、server 名称、远端工具名称或能力描述搜索；已知完整名称时，可用 `select:external_local_tools_query` 精确加载。精确加载只连接名称对应的 server，无法解析 server 时返回错误，不会扫描全部 server。匹配工具注册后会出现在下一轮模型请求中。

`tool_search` 会返回按相关性排序的候选工具，每项包含 `server`、`name`、`description`、`capabilities`、`matched`、`score` 和 `recommended`。模型应优先使用 `recommended=true` 且匹配原因最贴近用户任务的工具；如果没有发现合适 MCP 工具，应明确说明后再用可用本地上下文或模型知识回退。
