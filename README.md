...
- `/permissions`、`/perms`：显示当前权限模式和用户级、项目级、会话级规则摘要；支持 `mode strict|default|yolo|reset`。
...
使用 `/permissions` 查看当前模式；用 `/permissions mode strict|default|yolo` 临时切换，用 `/permissions mode reset` 恢复启动配置。切换仅对当前会话有效。
